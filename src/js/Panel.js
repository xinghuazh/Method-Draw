MD.Panel = function () {
  ...
  //$('#rect_rx')      .dragInput({ min: 0,    max: 100,   step:  1,  callback: editor.changeAttribute,     cursor: false  });
  $('#stroke_width').dragInput({ min: 0, max: 99, step: 1, callback: editor.changeAttribute, cursor: false, smallStep: 0.1, start: 1.5 });
  $('#angle').dragInput({ min: 0, max: 270, step: 90, callback: editor.changeRotationAngle, cursor: false, dragAdjust: 0.5 });
  $('#font_size').dragInput({ min: 1, max: 250, step: 1, callback: editor.text.changeFontSize, cursor: false, stepfunc: editor.stepFontSize, dragAdjust: .15 });
  $('#group_opacity').dragInput({ min: 0, max: 100, step: 5, callback: editor.changeAttribute, cursor: false, start: 100 });
  $('#item_blur').dragInput({ min: 0, max: 10, step: .1, callback: editor.changeBlur, cursor: false, start: 0 });
  ...
  function show(elem) {
    ...
    $.fn.dragInput.updateCursor($('#item_blur')[0])
  }
  ...
  function updateContextPanel(elems) {
    ...
    console.log("updateContextPanel", { elem, elems })

    const multiselected = elems.length > 1;

    var currentLayerName = svgCanvas.getCurrentDrawing().getCurrentLayerName();
    var currentMode = svgCanvas.getMode();
    console.log("currentLayerName", currentLayerName);
    console.log("currentMode", currentMode);

    $('.context_panel').hide();
    $("#align_panel").toggle(elem && !multiselected);
    ...
    if (elem !== null) {
      ...
      $('#item_blur').val(blurval);
      $.fn.dragInput.updateCursor(document.getElementById("item_blur"));
